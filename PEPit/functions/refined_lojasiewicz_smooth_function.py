...
class Refined_LojasiewiczSmoothFunction(Function):
    """
    The :class:`Refined_LojasiewiczSmoothFunction` class overwrites the `add_class_constraints` method
    ...
    The conditions implemented here are presented in [3, Proposition 3.2] (for alpha to be chosen).
    ...
    Example:
        ...
        >>> problem = PEP()
        >>> h = problem.declare_function(function_class=Refined_LojasiewiczSmoothFunction, mu=.5, L=1., alpha =.4)
