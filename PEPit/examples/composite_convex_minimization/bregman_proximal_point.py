...
def wc_bregman_proximal_point(gamma, n, verbose=True):
    ...
    where :math:`x_\\star` is a minimizer of :math:`F`, when :math:`D_h` is the Bregman distance generated by :math:`h`.
    ...
    **Algorithm**:
    ...
    For :math:`t \\in \\{0, \\dots, n-1\\}`,
        ...
        .. math::
            ...
            \\begin{eqnarray}
                x_{t+1} & = & \\arg\\min_{u \\in R^n} \\nabla f(x_t)^T(u - x_t) + \\frac{1}{\\gamma} D_h(u, x_t), \\\\
                D_h(x, y) & = & h(x) - h(y) - \\nabla h (y)^T(x - y).
            \\end{eqnarray}

    **Theoretical guarantee**: A **tight** empircal guarantee can be guessed from the numerics
    ...
    **References**: None.
    ...
    Examples:
        ...
        (PEP-it) Solver status: optimal (solver: SCS); optimal value: 0.06666565028497914
        *** Example file: worst-case performance of the Bregman Proximal Point in function values ***
            PEP-it guarantee:		 F(x_n)-F_* <= 0.0666657 Dh(x_*, x_0)
