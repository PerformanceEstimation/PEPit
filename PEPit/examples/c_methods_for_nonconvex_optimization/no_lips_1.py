...
def wc_no_lips1(L, gamma, n, verbose=True):
    ...
    That is, it computes the smallest possible :math:`\\tau(n,L, \\gamma)` such that the guarantee
       ...
       .. math:: \min_{1 \\leqslant i \\leqslant n} D_h(x_{i-1}, x_i) \\leqslant \\tau(n, L, \\gamma)  (F(x_0) - F(x_n))
    ...
    **Algorithms**:
        ...
        .. math:: D_h(x_{n}, x_{n+1}) = h(x_{n}) - h(x_{n+1}) - \\nabla h (x_{n})^T(x_{n} - x_{n+1})

    **Theoretical guarantees**:

    The **tight** theoretical upper bound is obtained in [1, Proposition 4.1]

        .. math:: \\tau(n, L, \\gamma) = \\frac{\\gamma}{n(1 - L\\gamma)}
    ...
    **References**:
    ...
    Example:
        >>> pepit_tau, theoretical_tau = wc_no_lips1(1, 1/2, 5)
        (PEP-it) Setting up the problem: size of the main PSD matrix: 19x19
        ...
        (PEP-it) Solver status: optimal (solver: SCS); optimal value: 0.2000007797552905
        *** Example file: worst-case performance of the NoLips in function values ***
            PEP-it guarantee:	 min_i Dh(x_(i+1)), x_(i)) <= 0.200001 (F(x_0) - F(x_n))
            Theoretical guarantee :	 min_i Dh(x_(i+1), x_(i)) <= 0.2 (F(x_0) - F(x_n))
    """
