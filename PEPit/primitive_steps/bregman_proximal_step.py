...
def bregman_proximal_step(sx0, mirror_map, min_function, gamma):
    ...
    That is, denoting by h(.) the mirror map, and f(.) the function to be minimized,
    it performs h'(x) = h'(x0) - gamma*f'(x), where h'(x) and h'(x0) are respectively
    (sub)gradients of the mirror map at x and x0, and f'(x) is a subgradient of f at x.

    Args:
        sx0: starting gradient sx0 (e.g., gradient at x0 of 'mirror_map'),
        mirror_map:mirror_map on which the (sub)gradient will be evaluated,
        min_function: min_function which we aim to minimize,
        gamma: step size.
