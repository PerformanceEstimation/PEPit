...
def inexact_proximal_step(z, f, gamma, opt='PD_gapII'):
    ...
    **Options:** The following options are available (a list of such choices is presented in [4]; we provide a reference
    for each of those choices below).
        ...
        This approximation requirement is used in two PEP-it example: accelerated inexact forward backward and
        ...
        one PEP-it example: accelerated inexact forward backward 2.
    ...
    if opt == 'PD_gapI':
        """
        This option constrain x and v to satisfy the following primal-dual gap requirement PD_gap(x,v) <= epsVar.
        """
        v = Point()
        w = Point()
        fw = Expression()
