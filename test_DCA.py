...
def wc_difference_of_convex_algorithm_loja(mu1, mu2, L1, L2, n, alpha = 0, wrapper="cvxpy", solver=None, verbose=1):
    # Instantiate PEP
    problem = PEP()

    # Declare a smooth convex function
    f1 = problem.declare_function(LojasiewiczSmoothFunction, L=L1, mu=mu1)
    f2 = problem.declare_function(LojasiewiczSmoothFunction, L=L2, mu=mu2)
    F = f1 - f2
    ...
    for i in range(n):
    	problem.set_performance_metric( (g1x-g2x)**2 )
    	problem.set_performance_metric( f1(x) - f2(x) - Fs)
    	y, _, _ = shifted_optimization_step(g2x, f1)
    	x = ( 1 + alpha ) * y - alpha * x
    	g1x, f1x = f1.oracle(x)
    	g2x, f1x = f2.oracle(x)
...
if __name__ == "__main__":
    ...
    pepit_tau, theory = wc_difference_of_convex_algorithm_loja(mu1, mu2, L1, L2, n, alpha = 1, wrapper="mosek", solver=None, verbose=verbose)
    print('*** DCA *** ')
    print('\tPEPit guarantee (std inequalities):\t min_(0<=i<=n) ||nabla f_1(x_(i))-nabla f_2(x_(i))||^2_2 <= {:.6} (F(x0)-Fs)'.format(pepit_tau))
    print('\tPEPit guarantee (std inequalities):\t min_(0<=i<=n) ||nabla f_1(x_(i))-nabla f_2(x_(i))||^2_2 <= {:.6} (F(x0)-Fs)'.format(theory))
